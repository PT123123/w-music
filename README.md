...
> **.idl 必须存成「UTF-8 with BOM」**：midlrt 对无 BOM 的文件按系统 ANSI 码页（本机 936）解码，
> 中文注释行尾的某些 UTF-8 字节会把下一行成员吞进注释——成员从 winmd 里静默消失（只在 C4819
> 警告里留一丝痕迹）。`dev-build.ps1` 现在会对非 ASCII 且无 BOM 的 .idl 直接报错拒绝构建。
...
- **新旧版本交接**：旧版本还在跑时启动新版本，新版本会让旧版本**优雅退出**——投递
  `WM_TRAY_GRACEFUL_EXIT`（`WM_APP+3`，`TrayIcon` 收到后走托盘"退出"的同一条清理路径：移除托盘
  图标、停推荐引擎子进程、正常关窗），等旧进程退干净（最多 6 秒）再把互斥量接过来自己启动。
  判定标准是 exe 路径（每个版本一个 workshop 目录，路径不同即"另一个构建"）；同一个 exe 双击
  第二次仍是老的"唤到前台"。0.1.21 及更早的旧实例不认识退出消息，新版本等超时后退回"唤到前台"，
  所以 **0.1.21 → 0.1.22 的这一次更新要手动退出一次旧实例**，之后所有版本间的交接全自动；
  全程不杀进程、不会双实例并行抢音频。交接过程在 `diag.log` 里有 `handoff ->` 系列痕迹。
...
- **推荐流（flow）是默认播放模式**：下一首不再实时问引擎，而是从**预取缓冲**同步出队——切歌零等待。
  缓冲由后台协程维护：磁盘缓存里的上一份 feed 先行垫底（离线种子，引擎冷启动的几秒到几分钟里
  「下一曲」也有货），引擎就绪后走 `POST /v1/feed/next` 按口味画像增量补货（低于 4 首补、上限 6 首，
  请求带缓冲内曲目做排除；本地再避开最近 30 首播过的与本会话跳过/差评过的）。流按**时间窗**组织
  （深夜/清晨/上午/午间/下午/傍晚/夜晚）：跨窗自动整流重算，在途的旧窗口答案作废，旧余量撑到新流
  就绪（绝不空窗）。全程**不弹任何选择框**；缓冲空时降级为顺序队列推进并诚实标注「推荐流还没就绪」，
  绝不阻塞。播放的曲子追加进队列，所以上一曲/下一曲在推荐流里也能自然回退/前进。
- **正在播放页新增「接下来」卡片**：推荐流模式显示预取缓冲的前 3 行预告（点谁播谁、它前面的预告一并
  出队；「换一个」跳过当前预告并后台补新），底行诚实说明状态（已预载几首 / 引擎预热中先用顺序顶上）。
  顺序类模式显示队列的非破坏性预览（`PlayQueue::PeekNext`，标「顺序」）；随机模式明确「不预告下一首」。
- **点赞/差评走手势，不打断播放**（正在播放页封面区）：左滑 = 差评（回传引擎 + **从当前播放队列移除**
  + 立即流向推荐流下一首，同曲预告一并清出缓冲），右滑 = 点赞（回传引擎加权口味画像），长按封面 = 差评；
  动作只给一行 2.6 秒的轻提示。曲库曲没有引擎的内容 hash，feedback 按文件路径发送，由引擎做只读解析
  （`/v1/feed/feedback` 接受可选 `file_path`，未分析过的文件 404、静默跳过）。
- **主动探索留在个性推荐页**：聚类 chips（预设 + 自动发现）点进去就是一份可播放列表；「与我正在听的
  相似」= "更多 like this"。推荐行的播放顺序由队列接管（修掉了推荐列表放完一首就停的 bug——`rec:`
  行不在曲库索引里，Next/MediaEnded 现在从旁路表解析）。
- 每行推荐带**匹配度**（引擎归一化分数）与**推荐理由**（真实计算的分组相似度）；文件自带 tag 时另起一行
  显示「文件标签：曲风 · 语种」——那是文件自己的声明，不是音频分析出的结论。
- **回答落在应用目录里**：`%LOCALAPPDATA%\w-music\recommend-cache.json`（`core::RecommendCache`），
  存的是引擎的**原始 JSON 回答**（最多 32 条 / 单条 ≤ 512 KiB），换安装、换构建都不丢。打开页面时
  先把缓存里的上一次结果画出来（chips、自动发现类别、列表都是），再在后台问引擎要最新的，
  列表上方写明「**以下是 N 分钟前的结果，正在向引擎要最新的**」；缓存与实时回答走**同一套解析函数**，
  所以同一段 JSON 不会被解释成两种结果。
- **慢在哪里要说清楚**：等待条不再只是一个转圈，而是分阶段说明——「正在启动本地推荐引擎（首次约 5～10 秒）」
  / 「引擎正在分析曲库，这个请求排在它后面」 / 「正在向本地引擎查询」。首次进过这一页后
  `settings.json` 记 `recommendPrewarm: true`，下次启动就在后台把引擎拉起来，冷启动的等待被挪到开机阶段。
- 缓存的**诚实边界**：每条回答按曲库规模打指纹（`lib:<曲目数>`），曲库变了旧答案不再充当新答案；
  「分析曲库」（真的入库了新特征）与「重置口味」都会清掉缓存；**与我正在听的相似**不缓存
  （种子是当前这一首歌，隔天问已经答非所问）。任何一份缓存结果的展示门槛是 **14 天**
  （`kMaxUsableMs`），超过就连「旧结果」也不画；每次动作仍然会向引擎要一次最新答案，
  缓存只负责**先有内容可看**，不负责冒充最新。请求耗时逐条写进 `diag.log`（`rec http /v1/…=NNNms`）。
